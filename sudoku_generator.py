...
class SudokuGenerator:
    ...
    def is_valid(self, row, col, num):
        box_row = row - (row % 3)
        box_col = col - (col % 3)
        if self.valid_in_box(box_row, box_col, num):
    ...
    def remove_cells(self):
        ...
        for i in range(0, self.removed_cells):
            while True:
                random_row = random.choice(list)
                random_col = random.choice(list)
